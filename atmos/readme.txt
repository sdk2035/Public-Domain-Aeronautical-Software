...
PROPERTIES OF THE STANDARD ATMOSPHERE                ~/PDAS/atmos/V17/readme.txt
...
archive file atmos.zip that may be downloaded from the PDAS web site.
  ...
  press.py        the program used to compute pressures at boundaries
...
Files associated with the "tables" program
  ...
  tables.pro      the source code in IDL (by Martin Shultz)
  tables.py       the source code in Python 3 (by Rich Kwan)
  tables2.py      the source code in Python 2 (by Rich Kwan)
  tables.zip      All of the output files (archived)
  bigtables.html  A web page (HTML) that has several tables of atmosphere data
  bigtables.py    The program (Python) that produced bigtables.html as output
  bigtables.f90   Same as bigtables.py, but coded in Modern Fortran 
...
The file hotcold.f90 is obviously designed to compute non-standard atmospheres.
...
perfect gas law. These non-standard atmospheres are only defined to 100,000 feet.
You can also define an atmosphere with a fixed temperature increment from
...
HTML Output - Also note the program bigtables.py that produces the web page
bigtables.html for display by a web browser. Here, instead of using 
...
keeping the numbers in neat columns. The file bigtables.html is quite long 
and is coded such that a readable portion is visible at any one time.
