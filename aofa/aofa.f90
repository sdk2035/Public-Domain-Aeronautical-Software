...
!+
     ...
     &                  WORK3,WORK4)
!          THIS SUBROUTINE COMPUTES THE INITIAL GUESS TO THE SOLUTION
!          AT THE NEXT X-STATION, AND CONTROLS THE MARCHING IN X.
      REAL   ME,MINF
!!!      LEVEL 2,U,V,W,H,P,ZI,UJP1,VJP1,WJP1,HJP1,PJP1,ZIJP1,UJM1,VJM1,
!!!     1 WJM1,HJM1,PJM1,ZIJM1,DELU,DELV,DELW,DELH,DELP,DELZI
      DIMENSION                                                         &
     &          U(NK,NL),V(NK,NL),W(NK,NL),H(NK,NL),P(NK,NL),ZI(NK,NL), &
...
! ***** 2/14/74 FIX ***** IF NOW .GT. 0 USE SLOPE (DIFFERENCE) TO
! ***** PREDICT NEXT SOLUTION ELSE USE CURRENT SOLUTION.  NOW WAS SET
! ***** IN ROUTINE BCIC.
  ...
  251 CONTINUE
   ...
   30 END DO
      DO 275 ITCOUN=1,40
      ITER = 0
!          CALL THE SUBROUTINE THAT ADVANCES THE SOLUTION TO THE
!          NEXT X-LOCATION USING A METHOD THAT IS IMPLICIT IN ETA.
!          AND ITERATIVE IN PHI.
...
!+
     ...
     &                  ET,FI,X,A,B,C,F,DEL,RTSIDE,WORK1,WORK2,         &
     &                  WORK3,WORK4)
!          THIS SUBROUTINE CONTROLS THE IMPLICIT IN ETA STEPS.
...
!!!     2       ,A,B,C,F,WORK1
     ...
     &     ,QZ33,QZ42
      EQUIVALENCE (DF,DFIL)
      DATA LFLAG/-1/
!          LFLAG DETERMINES WHETHER L GOES FROM 1 TO NL OR FROM NL TO 1.
      LFLAG=-LFLAG
      L=0
...
!          OBTAIN THE COEFFICIENTS FOR THE FI DERIVATIVES.
      IF (L.NE.1.AND.L.NE.NL)  GO TO 30
!          IF L = 1 OR NL THEN ALL PHI DERIVATIVES EXCEPT THOSE
!          INVOLVING W AND THE SECOND DERIVATIVES MUST BE ZERO.
!          DEFINE PARAMETERS SO THAT THIS HAPPENS AND SO THAT THERE
!          ARE NO PROBLEMS WITH SUBSCRIPTS OR DIVISION BY ZERO.
!          WHEN NL.NE.1 AND L=1 OR NL THEN LP1 IS USED IN OBTAINING
!          THE NONZERO PHI DERIVATIVES.  LM1 IS SET TO 1 AND IS NOT
!          INVOLVED WITH NONZERO TERMS.  LP1 MUST BE DEFINED PROPERLY.
      LM1=1
      LP1=1
      IF (NL.NE.1.AND.L.EQ.1)  LP1=L+1
      IF (NL.NE.1.AND.L.EQ.NL)  LP1=L-1
!          SET DFIL (= DELTA PHI) SO THAT CROSS DERIVATIVE TERMS ARE
!          HANDLED PROPERLY IN SETUPE.
                     !!! was 1.E+60
      DFIL = 1E35
      BETA1=0.
      BETA2=0.
   ...
   40 CONTINUE
      ...
      INETFI(L) = 0
!          OBTAIN THE STARTING VALUES OF THE PARAMETERS.
      DETKP1=ET(2)-ET(1)
...
!          THIS IS THE ETA LOOP FOR A SPECIFIC VALUE OF FI.
!          THE LIMITS ARE FROM 2 TO NK-1 SINCE THE BOUNDARY CONDITIONS
!          (K=1 AT THE BODY, K=NK AT THE SHOCK) ARE HANDLED
!          SEPARATELY.
      DO 100 K=2,NKM1
      KM1=K-1
      KP1=K+1
!          OBTAIN THE COEFFICIENTS FOR THE ETA DERIVATIVES.
      DETK=DETKP1
      DETKP1=ET(KP1)-ET(K)
...
!**********THE CONDITION DP/DETA=0 CAN BE INVOKED BY SETTING************
!**********SUBSON = .TRUE. , IT IS SUPRESSED WHEN SUBSON = .FALSE. *****
      SUBSON = .FALSE.
!**********SET SUBP1 = .TRUE. IF DP/DX IS TO BE EVALUATED EXPLICITLY****
!**********OR SET TO ZERO.  FOR DP/DX IMPLICIT SET SUBP1 = .FALSE. *****
      SUBP1  = .TRUE.
      RKM1=R
      R=RKP1
     ...
     &                DZ4)
!          COMPUTE SOME TERMS COMMON TO MANY OF THE DERIVATIVE
!          EXPRESSIONS.
      QZ1=ALPHA1*RHOKM1
      QZ2=ALPHA2*RHO
      QZ3=ALPHA3*RHOKP1
...
!**********TO SET DP/DX TO ZERO INSERT    DPDX = 0.    HERE.************
      DRUE=QZ4+QZ5+QZ6
      ...
      IF (L1NL)  GO TO 75
!          IF L = 1 OR NL AND THIS IS A NONZERO ANGLE OF ATTACK PROBLEM
!          THEN THE W DERIVATIVES AND SECOND DERIVATIVES WITH RESPECT
!          TO PHI ARE OBTAINED FROM ASSYMETRY OF W AND SYMMETRY OF THE
!          OTHER FUNCTIONS.
     ...
     &                            +ALPHA3*WJP1(KP1,LP1))*RDFIL
      ...
      GO TO 85
   75 CONTINUE
!          IF L DOES NOT = 1 OR NL THEN EVALUATE W-DERIVATIVES AND
!          CROSS DERIVATIVES STANDARDLY.
   ...
   85 CONTINUE
!          CALL THE SUBROUTINE THAT SETS UP THE MATRIX ELEMENTS
!          FOR EACH VALUE OF K.
...
!          CHECK FOR CONVERGENCE.
   ...
   95 CONTINUE
  100 END DO
!          OBTAIN THE PARAMETERS AND DERIVATIVES NEEDED IN THE
!          SHOCK BOUNDARY CONDITION EQUATIONS.
...
!          OBTAIN THE COEFFICIENTS FOR THE SHOCK BOUNDARY
!          CONDITION EQUATIONS.
      CALL SHOKBC (NK,NKM1,UJP1(1,L),VJP1(1,L),WJP1(1,L),HJP1(1,L),     &
     &             PJP1(1,L),ZIJP1(1,L),                                &
     &             ET,A,B,C,F)
!          OBTAIN THE PARAMETERS AND DERIVATIVES NEEDED IN THE
!          BODY BOUNDARY CONDITION EQUATIONS.
      R=XJP1*SINTC
...
!          OBTAIN THE COEFFICIENTS FOR THE BODY BOUNDARY
!          CONDITION EQUATIONS.
      CALL BODYBC (NK,UJP1(1,L),VJP1(1,L),WJP1(1,L),HJP1(1,L),PJP1(1,L),&
     &             ZIJP1(1,L),                                          &
     &             ET,A,B,C,F,VBJP1,HBJP1)
!          CALL THE ROUTINE THAT COMPUTES THE INCREMENTS TO BE
!          ADDED TO THE SOLUTION AT X(J).
  127 CONTINUE
  147 CONTINUE
...
! ***** FIX ADDED ON 2/15 ***** PROVIDES DIAGNOSTIC FOR NEGATIVE
! *****  ARGUMENTS BEING PASSED TO PROP AND OTHER NON-CONVERGING
! ***** CASES
 ...
 1000 FORMAT(2I10,E15.5)
  ...
  167 CONTINUE
      ...
      WJP1(1,L)=0.
      HJP1(1,L)=HBJP1
!          THE VARIABLES QZ1-6 WERE COMPUTED IN SUBROUTINE BODYBC
!          AND PASSED THROUGH COMMON QZBODY.
      PJP1(1,L) = PJP1(1,L)+(DELZI(2)*QZ2+DELV(2)*QZ3+DELV(3)*QZ33      &
     &                 +DELP(2)*QZ4+DELW(2)*QZ42+QZ6)*QZ1
  195 CONTINUE
  200 END DO
      RETURN
      END Subroutine Impeta

!+
      SUBROUTINE BODYBC (NK,U,V,W,H,P,ZI,ET,A,B,C,F,VBJP1,HBJP1)
!          THIS SUBROUTINE MODIFIES THE COEFFICIENTS AT K=2 TO TAKE
!          INTO ACCOUNT THE BOUNDARY CONDITIONS AT THE BODY (K=1).
!          THE VALUES OF THE SUBSCRIPTS ON THE PARAMETERS ARE
!          WITH RESPECT TO K=1.
      REAL   MU,ME
      REAL   MINF
      LOGICAL SUBSON,SUBP1
!!!      LEVEL 2,U,V,W,H,P,ZI,A,B,C,F
      DIMENSION                                                         &
     &      U(NK),V(NK),W(NK),H(NK),P(NK),ZI(NK),                       &
...
!          IF V AT THE CONE DOES NOT = 0, USE THE CONTINUITY EQUATION.
      ...
      GO TO 75
   50 CONTINUE
!          IF V AT THE CONE = 0, USE THE V-MOMENTUM EQUATION.
      QZ1=1.
      QZ2=0.
      QZ3=0.
      QZ4=1.
...
!!!      LEVEL 2,U,V,W,H,P,ZI,A,B,C,F
  ...
  200 END DO
     ...
     &        -QZ16*(QZ12*QZ12+QZ13*QZ13+QZ14*QZ14)
      ...
      RETURN
      END Subroutine ShokBC

!+
      SUBROUTINE SETUPE (NK,NKM1,U,V,W,H,P,ZI,ET,A,B,C,F)
!          THIS SUBROUTINE SETS UP THE COEFFICINT MATRIX FOR THE
!          IMPLICIT IN ETA STEPS.
      REAL   MU,ME
      REAL   MINF
      LOGICAL SUBSON,SUBP1
!!!      LEVEL 2,U,V,W,H,P,ZI,A,B,C,F
     ...
     &         MINF,ALFA,SINALF,CTCA,STSA,STCA,CTSA,PINF,HBAR,SPROP
      COMMON /VARY/XJM1,XJ,XJP1,DX,DXJM1,JM1,J,JP1
!          THE VARIABLES QZ= ARE USED TO STORE EXPRESSIONS COMMON TO
!          MANY TERMS
      QZ0=ALPHA1*R*ZI(K)
...
!          IF DP/DX IS EVALUATED EXPLICITLY OR SET TO ZERO REDEFINE B25K
      IF (SUBP1)                                                        &
...
!          IF DP/DX IS EVALUATED EXPLICITLY OR SET TO ZERO REDEFINE B65K
      IF (SUBP1)                                                        &
...
!          NOW THE VISCOUS HEAT CONDUCTION TERMS
     ...
     &        +MU*QZ42*ME/REINF
!          NOW THE VISCOUS DISSIPATION TERMS
...
!          ADD IN THE CONTRIBUTIONS FROM THE PHI-DERIVATIVE TERMS.
...
!          ADD IN SOME TERMS TO THE ENERGY EQUATION THAT HAD BEEN
!          FORGOTTEN.
...
!          IF DP/DETA IS SET TO ZERO, REDEFINE A FEW COEFFICIENTS.
      DO 500 I=1,6
      A(3,I,K) = A(5,I,K)
      B(3,I,K) = B(5,I,K)
      C(3,I,K) = C(5,I,K)
...
!          THIS SUBROUTINE READS IN THE MESH DISTRIBUTION FOR X, ET,
!          AND FI.  THE BOUNDARY CONDITIONS AND THE INITIAL CONDITIONS
!          ARE ALSO READ IN.
!          TAPE3 IS THE INPUT TAPE WHEN THE INITIAL CONDITIONS ARE READ
!          FROM TAPE
      REAL   MU
...
!***** 4/17/74 *****
!
!     COMMON/INTGRT/ CONTAINS THE INTEGRAL UP TO THE X-STATION OF THE
!     FORCES ON THE CONE.
!     THE DATA STATEMENT IS USED TO INITIALIZE THE INTERGRATION.  NOTE
!     IF NOT READ OFF OF TAPE THEY ALL ARE ZERO
!
!!!      COMMON/INTGRT/ INIT,MF,MY,FNX,FAX,FAY,FNY,FNF
!!!      REAL MF,MY
!***** FIX ADDED 2/14/74 ***** NOW DETERMINES WHEN THE INITIAL GUESS
!***** WILL BE CURRENT VALUE AND WHEN IT WILL BE DIFFERENCE (SLOPE)
!***** PREDICTED IN ROUTINE FLOFLD.  PRESENT SCHEME CALLS FOR WAIT OF 2
!***** STEPS IF INPUT COMES FROM CARDS
      COMMON/PREDICT/NOW,X0
!!!      DATA INIT,MF,MY,FNX,FAX,FAY,FNY,FNF/0,7*0./
      NOW=0
      X0=0.
      ...
      NOW=-10
      DO 100 L=1,NL
!          READ IN A VALUE FOR FI AND THE CORRESPONDING VALUE
!          FOR ZI.  (ZI IS NOT A FUNCTION OF ETA.)
! ***** 3/20/74 *****
! IREAD = 0 THE REAL INITIAL (1ST RUN) CONDITIONS COME OFF OF TAPE7
! IREAD .LT. 0 THE DATA WILL BE ON CARDS
      IF (IREAD.EQ.0) GO TO 40
      READ (5,1010) FI(L),ZI(1,L)
      FI(L)=FI(L)*.0174532925199433
!          READ IN THE INITIAL VALUES OF THE SOLUTION (THE VALUES AT
!          X=X(1)) ALONG A RAY NORMAL TO THE BODY.
  ...
  105 CONTINUE
!          READ IN THE DISTRIBUTION OF X-STATIONS ALONG THE BODY
!          AT WHICH THE SOLUTION WILL BE OBTAINED.
      NJOR6=NJ
      IF (NJ.GT.6)  NJOR6=6
      READ (5,1010) (X(J),J=1,NJOR6)
      IF (NJ.LE.2)  GO TO 180
      IF (X(2).LT.X(3).AND.NJ.GT.6)  READ (5,1010) (X(J),J=7,NJ)
!          IF ALL OF THE X-STATIONS ARE TO BE READ IN, READ IN THE REST
!          OF THEM
      IF (X(2).LT.X(3))  GO TO 180
      DX=X(2)-X(1)
!          THE REST OF THE X-STATIONS CAN BE GENERATED FROM THE FIRST
!          ONE.  EACH SUCCESSIVE X-STATION IS MADE PROPORTIONAL TO THE
!          PREVIOUS ONE.
! (4/74)   IF X-STATIONS ARE TO BE CALCULATED, INPUT A CARD WITH THE
!          PERCENTAGE GAIN (AS FRACTION) AT EACH STEP
      READ (5,1010) XGAIN
      DO 175 J=2,NJ
...
!          READ IN THE INITIAL PLANE FROM TAPE3.
      EPS = 1.E-10
      READ (3) QZ,IQZ
  ...
  140 CONTINUE
      ...
      X0=XX
  145 CONTINUE
!          READ IN THE VALUES OF V AND H ALONG THE BODY AT VARIOUS
!          L-J (ETA-X) GRID POINTS.  LINEAR INTERPOLATION WILL BE USED
!          TO OBTAIN VALUES OF VB AND HB WHERE THEY ARE NOT SPECIFIED.
  200 CONTINUE
      READ (5,1020) L
  250 CONTINUE
...
!          DEFINE THE SOLUTION SO THAT THE X-DERIVATIVES FOR THE
!          FIRST X-STEP WILL BE ZERO.
      DO 800 I2=1,NL
      DO 800 I1=1,NK
      UJM1(I1,I2)=U(I1,I2)
      VJM1(I1,I2)=V(I1,I2)
      ...
      HJM1(I1,I2)=H(I1,I2)
      PJM1(I1,I2)=P(I1,I2)
!          ZI IS NOT A FUNCTION OF ETA (I.E. I1).
      ZI(I1,I2)=ZI(1,I2)
...
! THIS ROUTINE IS USED TO MAKE SURE INPUTS ON CARDS SATISFY THE
! RELATIONSHIP--
!
!     HTOT.LE.HBAR       AND   HTOT.EQ.HBAR  AT SHOCK BOUNDARY
      COMMON/CONST/DUM(4),ME,DOM(4),MINK,DIM(7),HBAR,DEM
!!!      LEVEL 2,H,U,V,W
      DIMENSION                                                         &
     &      H(NK,NL),U(NK,NL),V(NK,NL),W(NK,NL)
      REAL ME,MINF,MED2
      NKM1=NK-1
      MED2=ME/2.
      DO 10 L=1,NL
...
!LEQ  LINEAR EQUATIONS SOLUTIONS  FORTRAN II VERSION
!     SOLVE A SYSTEM OF LINEAR EQUATIONS OF THE FORM AX=B BY A MODIFIED
!     GAUSS ELIMINATION SCHEME
!
!     NEQS = NUMBER OF EQUATIONS AND UNKNOWNS
!     NSOLNS = NUMBER OF VECTOR SOLUTIONS DESIRED
!     IA = NUMBER OF ROWS OF A AS DEFINED BY DIMENSION STATEMENT ENTRY
!     IB = NUMBER OF ROWS OF B AS DEFINED BY DIMENSION STATEMENT ENTRY
!     ADET = DETERMINANT OF A, AFTER EXIT FROM LEQ
!
      DIMENSION A(IA,IA),B(IB,IB)
      NSIZ = NEQS
      NBSIZ = NSOLNS
!     START SYSTEM REDUCTION
      NUMSYS=NSIZ-1
      DO 14 I=1,NUMSYS
      NN=I+1
      BIG=A(I,I)
      NBGRW=I
      BG=1.0/BIG
!     ELIMINATE UNKNOWNS FROM FIRST COLUMN OF CURRENT SYSTEM
   10 DO 13 K=NN,NSIZ
!     COMPUTE PIVOTAL MULTIPLIER
      PMULT=-A(K,I)*BG
!     APPLY PMULT TO ALL COLUMNS OF THE CURRENT A-MATRIX ROW
      DO 11 J=NN,NSIZ
   11 A(K,J)=PMULT*A(I,J)+A(K,J)
!     APPLY PMULT TO ALL COLUMNS OF MATRIX B
      DO 12 L=1,NBSIZ
   12 B(K,L)=PMULT*B(I,L)+B(K,L)
   13 END DO
   14 END DO
!     DO BACK SUBSTITUTION
!     WITH B-MATRIX COLUMN = NCOLB
   50 DO 15 NCOLB=1,NBSIZ
!     DO FOR ROW = NROW
      DO 19 I=1,NSIZ
      NROW=NSIZ+1-I
      TEMP=0.0
!     NUMBER OF PREVIOUSLY COMPUTED UNKNOWNS = NXS
      NXS=NSIZ-NROW
!     ARE WE DOING THE BOTTOM ROW
      IF(NXS) 16,17,16
!     NO
   16 DO 18 K=1,NXS
      KK=NSIZ+1-K
...
!     HAVE WE FINISHED ALL ROWS FOR B-MATRIX COLUMN = NCOLB
   19 END DO
!     YES
!     HAVE WE JUST FINISHED WITH B-MATRIX COLUMN NCOLB=NSIZ
   15 END DO
!     YES
!     WE ARE ALL DONE NOW
!     WHEW...
...
!          THIS SUBROUTINE USES QUADRATIC INTERPOLATION TO OBTAIN THE
!          INITIAL CONDITIONS AT A NEW MESH DISTRIBUTION.
!!!      LEVEL 2,U,UNEWE,UNEWEF,V,VNEWE,VNEWEF,W,WNEWE,WNEWEF,P,PNEWE,
!!!     1 PNEWEF,H,HNEWE,HNEWEF,ZI,ZINEWE,ZINWEF,UNEW,VNEW,WNEW,PNEW,HNE
!!!     2 ZINEW
      DIMENSION                                                         &
...
USE Intgrt
      REAL   MU,ME
      REAL   MINF
!          THIS SUBROUTINE OUTPUTS THE SOLUTION AT THE ETA-PHI
!          GRID OF POINTS FOR A SPECIFIC VALUE OF X.
!          THE SOLUTION IS PRINTED, WRITTEN ON TAPE4, AND WRITTEN ON
!          TAPE2.  TAPE2 CAN BE SET UP AS THE CARD PUNCH SINCE THE
!          FORMAT THAT IS USED FOR TAPE2 IS COMPATIBLE WITH THE FORMAT
!          THAT SUBROUTINE BCIC USES TO READ THE SOLUTION FROM CARDS.
!          HOWEVER FOR RESTART PURPOSES IT IS ADVISABLE TO USE THE
!          WRITE ON TAPE4 READ FROM TAPE3 COMBINATION.
...
!          CONVERT NORMAL COORDINATE TO DIMENSIONED QUANTITY.
      QZ1=ZI(1,L)
      DO 100 K=1,NK
      Y(K)=ET(K)*QZ1
...
!          STATIONS TO BE PRINTED CAN BE CONTROLLED BY AN APPROPRIATE
!          STATEMENT HERE.
  ...
  500 CONTINUE
      RETURN
 1100 FORMAT ("1      SOLUTION AT X = ",E15.5)
 1200 FORMAT(///," AT FI=",F8.2," CUINF=",E13.5," CWINF=",E13.5,        &
     & " STINF=",E13.5," SHOCK=",E13.5," WALL P=",E13.5)
 1250 FORMAT(//,8X,"Y",13X,"U",13X,"V",13X,"W",13X,"P",13X,"H",         &
     &       11X,"H TOTAL", 9X,"CPS",11X,"CPP",//)
 1300 FORMAT (9E14.5)
 2100 FORMAT (6E12.5)
...
! ---------------------------------------------------------------------------
!PURPOSE - This subroutine obtains the fluid properties.
      REAL::    mu,me, minf
     COMMON /CONST/costc,sintc,reinf,prinf,me,rreinf,rprre,rreme,gm2,  &
     &         minf,alfa,sinalf,ctca,stsa,stca,ctsa,pinf,hbar,sprop
!----------------------------------------------------------------------------
...
SUBROUTINE PropMc(h,p,rho,drp,drh,mu,dmh,con,dch)
! ---------------------------------------------------------------------------
! PURPOSE - PROPMC is an entry point used to obtain mu and con.
      REAL::    mu,me, minf
     COMMON /CONST/costc,sintc,reinf,prinf,me,rreinf,rprre,rreme,gm2,  &
     &         minf,alfa,sinalf,ctca,stsa,stca,ctsa,pinf,hbar,sprop
!----------------------------------------------------------------------------
      ...
      dch=dmh
      RETURN
END Subroutine PropMc

SUBROUTINE PROPRO(H,P,RHO,DRP,DRH,MU,DMH,CON,DCH)
! ---------------------------------------------------------------------------
! PURPOSE - PROPRO IS AN ENTRY POINT USED TO OBTAIN RHO
      REAL::    mu,me, minf
     COMMON /CONST/costc,sintc,reinf,prinf,me,rreinf,rprre,rreme,gm2,  &
     &         minf,alfa,sinalf,ctca,stsa,stca,ctsa,pinf,hbar,sprop
!----------------------------------------------------------------------------
...
!+
     ...
     &                   WORK3,WORK4)
!          THIS SUBROUTINE USES A BLOCK TRIDIAGONAL ALGORITHM TO SOLVE
!          THE SYSTEM OF LINEAR EQUATIONS.  SEE--ANALYSIS OF NUMERICAL
!          METHODS BY ISAACSON AND KELLER (1966) PP.58,59,60.
!!!      LEVEL 2,A,B,C,F,DELU,DELV,DELW,DELH,DELP,DELZI,WORK1
      DIMENSION                                                         &
     &      A(N,N,NP),B(N,N,NP),C(N,N,NP),F(N,NP),                      &
...
!+
      SUBROUTINE Integrate(TAOX,TAOY,TAOF,FI,X,NL)
USE Intgrt
!***** 3/74 *****
! THIS ROUTINE DOES A SIMPLE DIFFERENCE INTEGRAL OF THE FORCES ACTING
! ON THE CONE.
!  NOTATION -- F=FORCE  M=MOMENT  N=NORMAL  A=AXIAL  X=X COMPONET CAUSED
!  Y= Y COMPONET CAUSED  F= FI COMPONET CAUSED PRE = PREVIOUS STEP
!  INT= INTERMEDIATE STEP  P=PARTIAL (THIS RUN ONLY)
      DIMENSION TAOX(NL),TAOY(NL),TAOF(NL),FI(NL)
      COMMON/PARM/THETA
      REAL MFINT,MFPRE,MYINT,MYPRE,MTOT
!!!      REAL MF,MY
!!!      COMMON/INTGRT/INIT,MF,MY,FNX,FAX,FAY,FNY,FNF
   ...
   10 END DO
     ...
     &                  FNX, FAX, FNY, FAY, FNF, MF, MY,                &
     &                  FN,FA,MTOT
 1000 FORMAT(//////,                                                    &
     & " INTEGRATION OF FORCES FOR X FINAL = ",E12.5,/,                 &
     &24X, "       X COMPONET              Y COMPONET      ",           &
     & " FI COMPONET   M COMP. FI  M COMP. Y ",/                        &
     &24X, "   NORMAL      AXIAL       NORMAL      AXIAL       NORMAL",/&
     &     " XINIT = START OF RUN   ",7E12.3,/                          &
     &     " XINIT = START OF SERIES",7E12.3,/                          &
     &     " TOTAL NORMAL FORCES =  ",E12.3,                            &
     &     " TOTAL AXIAL FORCES  =  ",E12.3,                            &
     &     " TOTAL MOMENT        =  ",E12.3,/////)
  100 CONTINUE
      XPRE =X
      MYPRE=MYINT
...
!          THE MAXIMUM SIZE OF ALL ARRAYS IS SPECIFIED IN THE
!          DIMENSION STATEMENT.  IF MAXJ,MAXK,MAXL DENOTE THE
!          UPPER BOUNDS FOR J,K,L, AND MAXKL DENOTES THE LARGER
!          OF MAXK AND MAXL THEN THE ARRAYS ARE DIMENSIONED
!          DEPVAR(MAXK*MAXL,18)
!          ABCFD(MAXKL,118)
!          ET(MAXK),FI(MAXL),X(MAXJ)
!          ETNEW(MAXKL),FINEW(MAXKL)
!          INETFI(MAXL),INFIET(MAXK)
!          VB(MAXJ,MAXL),HB(MAXJ,MAXL)
!          COEFF(MAXJ,MAXL),DEL((MAXK-1)*6),RTSIDE((MAXK-1)*6),
!          WORK2(1),WORK3(1),WORK4(1)
!          IF MAXK IS GREATER THAN 108 THEN THE DIMENSION OF Y IN
!          BLANK COMMON SHOULD BE MAXK.
!
!
!          TAPE2 IS AN OUTPUT UNIT, SEE SUBROUTINE OUTPUT.
!          TAPE3 AND TAPE4 ARE INPUT AND OUTPUT UNITS,
!          SEE SUBROUTINES BCIC AND OUTPUT.
!
!
!          PROBLEM PARAMETERS ARE READ USING NAMELIST IN THE MAIN
!          ROUTINE.  FOR OTHER INPUT SEE SUBROUTINE BCIC.
!
!
!          THERE ARE A FEW UNCONVENTIONAL (AND FRUSTRATING IF FORGOTTEN)
!          THINGS IN THE PROGRAM.  SEE SUBROUTINES FLOFLD, IMPETA, BCIC,
!          AND OUTPUT, THE COMMENTS ARE INDICATED WITH ASTERISKS ****.
...
!
!          *** APRIL 1, 1974 *** THE ORDER OF DECLARION IN LARGE IS
!          CRITICAL. IN CALL TO FLOFLD ETNEW EXTENDS THRU FINEW AND INTO
!          34 COLLUMNS OF ABCFD.
!
      COMMON/LCM/LARGE(84200)
!!!      LEVEL 2,LARGE
      EQUIVALENCE (LARGE(1),DEPVAR),(LARGE(68401),ETNEW),               &
     & (LARGE(68501),FINEW),(LARGE(68601),ABCFD),(LARGE(80401),COEFF)
!!!      LEVEL 2,DEPVAR,ETNEW,FINEW,ABCFD,COEFF
      DIMENSION                                                         &
     &      DEPVAR(3800,18),ETNEW(100),FINEW(100),ABCFD(100,118)        &
     &  ,COEFF(100,38)
      DIMENSION                                                         &
...
!          THE FOLLOWING PARAMETERS ARE INPUT WITH NAMELIST
!          GAMMA = RATIO OF SPECIFIC HEATS
!          MINF  = FREESTREAM MACH NUMBER
!          THETAC= CONE HALF-ANGLE (DEG)
!          REINF = FREESTREAM REYNOLDS NUMBER (/FT)
!          PRINF = FREESTREAM PRANDTL NUMBER
!          ALFA  = ANGLE OF ATTACK (DEG)
!          PINF  = DIMENSIONLESS FREESTREAM PRESSURE
!          SPROP = SUTHERLAND CONSTANT USED IN VISCOSITY LAW
!          NJ    = NUMBER OF X-STATIONS
!          NK    = NUMBER OF Y-STATIONS
!          NL    = NUMBER OF PHI-STATIONS
!          MOD   = .TRUE. OR .FALSE. DEPENDING ON WHETHER THE INITIAL
!                  CONDITIONS ARE TO BE MODIFIED OR NOT, DEFAULT=.FALSE.
!          ITAPE = 0 NO OUTPUT ON TAPE2, THIS IS THE DEFAULT VALUE
!                = N OUTPUT SOLUTION ON TAPE2 EVERY NTH X-STATION.
!          IETASK =EVERY IETASK TH POINT IS PRINTED OUT (3/74)
!          IPHISK(J) = 0 TO SUPPRESS PRINTING OF THE J TH PHI STATION
!                      ELSE DO NOT INPUT (3/74)
!!!      NAMELIST /INPUT/ GAMMA,MINF,THETAC,REINF,PRINF,ALFA,              &
!!!     &                 PINF,SPROP,NJ,NK,NL,MOD,ITAPE                    &
!!!     &                  ,IETASK,IPHISK
      NAMELIST /INPUT/ ALFA,GAMMA,IETASK,IPHISK,ITAPE,MINF,MOD,NJ,       &
     & NK,NL,PINF,PRINF,REINF,SPROP,THETAC
...
!----------------------------------------------------------------------------
      ...
      MOD = .FALSE.
      ITAPE=0
      WRITE(*,*) 'aofa - three-dimensional supersonic viscous flow'
      DO
      ...
      END DO
      OPEN(UNIT=6,FILE='aofa.out',STATUS='REPLACE',ACTION='WRITE')
      OPEN(UNIT=2,FILE='aofa.tp2',STATUS='REPLACE',ACTION='WRITE')
      OPEN(UNIT=4,FILE='aofa.tp4',STATUS='REPLACE',ACTION='READWRITE',FORM='UNFORMATTED')
      READ (5,INPUT)
      WRITE (6,INPUT)
      ...
      STCA=SINTC*COSALF
      CTSA=COSTC*SINALF
!          OBTAIN THE BOUNDARY CONDITIONS AND THE INITIAL CONDITIONS.
      CALL BCIC(NJ,NK,NL,                                               &
...
!          OUTPUT THE INITIAL CONDITIONS BEFORE THEY ARE MODIFIED.
!... Original code
!      CALL OUTPUT(NK,NL,X,ET,FI,                                        &
!     &DEPVAR(1,1),DEPVAR(1,4),DEPVAR(1,7),DEPVAR(1,10),DEPVAR(1,13),    &
!     &DEPVAR(1,16))
!... Revised code
      CALL OUTPUT(NK,NL,X(1),ET,FI,                                        &
     &DEPVAR(1:,1:),DEPVAR(1:,4:),DEPVAR(1:,7:),DEPVAR(1:,10:),DEPVAR(1:,13:),    &
     &DEPVAR(1:,16:))
!          IF THE SOLUTION IS TO BE MODIFIED READ IN A NEW VALUE FOR
!          NK AND/OR NL.
      READ (5,5010) NEWK,NEWL
 5010 FORMAT (2I5)
      IF (NEWK.LE.0) GO TO 50
      NEWNK = NEWK
!          READ IN A NEW NORMAL DISTRIBUTION.
 ...
 5020 FORMAT (6E12.4)
   ...
   55 CONTINUE
      ...
      NEWNL = NEWL
!          READ IN A NEW CIRCUMFERENTIAL DISTRIBUTION (DEG).
   ...
   72 END DO
   75 CONTINUE
!          INTERPOLATE TO OBTAIN THE SOLUTION AT THE NEW MESH
!          DISTRIBUTION FOR EACH OF THE INPUT PLANES.
